...
spec:
  ...
  extraArgs:
    ...
    - "Llama-3.2-3B-Instruct-prefill"

    - "--decode-model-labels"

    - "Llama-3.2-3B-Instruct-decode"
...
The request will be automatically routed through the prefill nodes for the initial processing, and then the decode nodes will handle token generation, with KV cache seamlessly transferred between them.
...
2. **VLLMRuntime pods not starting**: Check resource availability and GPU access.
